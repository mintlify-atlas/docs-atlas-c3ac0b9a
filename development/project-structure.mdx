...
- **`run(filename, input_text)`** — resolves `filename` to an absolute path, calls `parse_cantor_file` to build a parse tree, creates a `CantorInterpreter`, visits the tree, encodes the raw input text with `encode_input_text`, and returns the integer result of calling `interpreter.evaluate(encoded_input)`.
